...
# 获取三羊数据
def get_skin_86_market(page=1,page_size=10):
    try:
        # 设置请求的URL
...
def get_skin_86_market_all(page_size=10,limit_page=10):
    file_name=data_path+skin_86_path
    ...
    while True:
        ...
        time.sleep(1)
        page_data=get_skin_86_market(page,page_size)

        if page_data is None or page>limit_page:
            print("获取数据完成")
            break

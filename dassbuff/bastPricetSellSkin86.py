           ...
           if len(json_data)>0:
                ...
                for create_target in json_data:
                    if create_target['totalSales']>=3 :
                        ...
                        if create_target['dmarket_sale_Price']>1 and create_target['dmarket_sale_Price']<5 and create_target['dm_buy_buff_sale_avg_rate']>0.10 and create_target['price_alter_percentage_7d']<20:
                            buy_flag=True
                            us_price= us_price+1
                            buy_it_num=5
                        elif '武器箱' not in create_target['category_group_name'] and create_target['dmarket_sale_Price']>=5 and create_target['dmarket_sale_Price']<15 and create_target['dm_buy_buff_sale_avg_rate']>0.10  and create_target['price_alter_percentage_7d']<15:
                            buy_flag=True
                            us_price= us_price+2
                            buy_it_num=2
           ...
           if len(json_data)>0:
                ...
                for create_target in json_data:
                    if create_target['totalSales']>=3 :
                        ...
                        if create_target['offer_price']>1 and create_target['offer_price']<5 and create_target['dm_buy_buff_sale_min_rate']>0.04 and create_target['price_alter_percentage_7d']<20:
                            buy_flag=True
                            us_price= us_price+1
                            buy_it_num=2
                        elif '武器箱' not in create_target['category_group_name'] and create_target['offer_price']>=5 and create_target['offer_price']<10 and create_target['dm_buy_buff_sale_min_rate']>0.04  and create_target['price_alter_percentage_7d']<15:
                            buy_flag=True
                            us_price= us_price+2
                            buy_it_num=1
...
def sync_data(limit_page=10,page=1,page_size=10,price_start=1,price_end=200,selling_num_start=100,platform='YP'):
    exchange_rate=find_us_exchange()
    print("当前的美元汇率是："+str(exchange_rate))
    # # 初始化数据
    Skin86BaseData.get_skin_86_market_all(file_name= buff_file,limit_page=limit_page,page=page,page_size=page_size,price_start=price_start,price_end=price_end,selling_num_start=selling_num_start,platform=platform)
    thread_size=5
...
# def start():    
    ...
    print("当前的美元汇率是："+str(exchange_rate))
    # # 初始化数据
    Skin86BaseData.get_skin_86_market_all(file_name= buff_file,limit_page=100,page=0,page_size=50,price_start=5,price_end=2000,selling_num_start=100,platform='YP')
    thread_size=5
    process_file_in_threads(thread_size,exchange_rate)
    #导出市场数据
    export_market_data()
